...
bool SfmBundle::cleanup(sfmData::SfMData & sfmData)
{
    ...
    const std::size_t nbOutliersResidualErr = RemoveOutliers_PixelResidualError(sfmData, _featureConstraint, _maxReprojectionError, _minTrackLength);

    // Remove landmarks without enough observed parallax
    const std::size_t nbOutliersAngleErr = RemoveOutliers_AngleError(sfmData, _minAngleForLandmark);
