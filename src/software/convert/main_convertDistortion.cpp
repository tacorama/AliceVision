...
bool convert(std::shared_ptr<camera::Undistortion> & undistortion, const camera::IntrinsicBase & intrinsic)
{
    std::vector<calibration::PointPair> ppts;
    for (int i = 0; i < intrinsic.h(); i += 5)
    {
        for (int j = 0; j < intrinsic.w(); j += 5)
        {
            calibration::PointPair ppt;
            ppt.undistortedPoint.x() = j;
            ppt.undistortedPoint.y() = i;

            ppt.distortedPoint = intrinsic.get_d_pixel(ppt.undistortedPoint);

            if (ppt.distortedPoint.x() < -10) continue;
            if (ppt.distortedPoint.y() < -10) continue;
            if (ppt.distortedPoint.x() > intrinsic.w() + 10) continue;
            if (ppt.distortedPoint.y() > intrinsic.h() + 10) continue;
    ...
    for (auto & pairIntrinsic : sfmData.getIntrinsics())
    {
        ...
        if (from == "distortion")
        {
            if (to == "undistortion")
            {
                ...
                if (convert(undistortion, *intrinsic))
                {
